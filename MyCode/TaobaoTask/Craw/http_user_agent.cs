   ...
   public class http_user_agent
    {
        ...
        {
            var tIndex = index++;
            if (tIndex < 0 || tIndex >= Dictionary.Count)
            {
                tIndex =index= 0;
            }
            return Dictionary.ElementAt(tIndex).Value;
        }
   static http_user_agent(){
      string[] useragents = new string[]{  "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/536.11 (KHTML, like Gecko) Chrome/20.0.1132.11 TaoBrowser/2.0 Safari/536.11",
...
"Mozilla/5.0 (Linux; U; Android 2.2.1; zh-cn; HTC_Wildfire_A3333 Build/FRG83D) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1",
      ...
      {
          if (!Dictionary.ContainsKey(useragent))
          {
