...
namespace HtmlAgilityPack
{
	...
	public class HtmlWeb
	{
        ...
        private bool _IsTextContent;//zfr
		/// <summary>
        ...
        private bool IsTextContentF(string contentType)
        {
            return contentType.ToLower().StartsWith("text/");
        }
		...
		private HttpStatusCode Get(Uri uri, string method, string path, HtmlDocument doc)
		{
			...
			req.Method = method;
			req.UserAgent = " Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9) Gecko/2008052906 Firefox/3.0";
...
//				}
			...
			catch (WebException we)
			{
				...
				if (resp == null)
				{
                    ...
                    return HttpStatusCode.GatewayTimeout;//zfr
					//throw;
				}
			}
			catch(Exception)
            ...
            this._IsTextContent = IsTextContentF(resp.ContentType);
			System.Text.Encoding respenc=null;
            //zfr编码
            if ((resp.CharacterSet != null) && (resp.CharacterSet.Length > 0))
            {
                string tmp = resp.CharacterSet.Replace(" ", "").Replace("\"", "").Replace("'", "");//zfr
                if (tmp.IndexOf(",") != -1)//处理" gbk,utf-8"
                   tmp=tmp.Substring(0,tmp.IndexOf(","));
                respenc = System.Text.Encoding.GetEncoding(tmp);
            }
            ...
            if (s != null)
			{
				...
				else
				{
                    ...
                    if ((doc != null) && (this._IsTextContent))
					{
						if (respenc != null)
                        {
                            switch (respenc.WebName.ToLower())
                            {
                                case "iso-8859-1": 
                                   respenc = System.Text.Encoding.GetEncoding("iso-8859-1");//
                                    StreamReader sr = new StreamReader(s, respenc, false);
                                    doc.LoadHtml(GetEncoding8859String(sr.ReadToEnd()));//zfr
                                    break;
                                default: 
                                    doc.Load(s, respenc, true);//zfr
                                    break;
                            }
                            //doc.Load(s, respenc, true);//zfr
						...
						{
							//doc.Load(s, true);
                            doc.Load(s, System.Text.Encoding.UTF8,true);//zfr
						}
					}
				}
				//resp.Close();
        ...
        /// <returns></returns>
        public static string GetEncoding8859String(string srcString)
        {
            //
            Encoding srcEncode = Encoding.GetEncoding("iso-8859-1");
            Encoding dstEncode;
            if (System.Text.RegularExpressions.Regex.IsMatch(srcString, "charset=gb2312", System.Text.RegularExpressions.RegexOptions.IgnoreCase))
            {
                dstEncode = Encoding.GetEncoding("gb2312");
            }
            else
                dstEncode = Encoding.UTF8;
            //byte[] srcBytes = srcEncode.GetBytes(srcString);//用iso-8859-1去转换源字符串
           // byte[] dstBytes = Encoding.Convert(srcEncode, dstEncode, srcBytes);//但是，是从gb2312转到unicode的
            ...
            return dstEncode.GetString(srcEncode.GetBytes(srcString));//
        }
